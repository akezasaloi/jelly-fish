...
alter table parent_details add column gender VARCHAR (10);
alter table parent_details add column national_id VARCHAR (10);
select * from parent_details;

insert into parent_details (parent_id, parent_name, contact_information, location, estimated_income, gender, national_id)
...
select COUNT(*) 
	...
	from parent_details where LEFT(parent_name, 1) in ('k', 'K'); 